...
from avion.utils.evaluation_ek100cls import (
    get_marginal_indexes,
    get_mean_accuracy,
    marginalize,
)
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="AVION finetune ek100 cls", add_help=False
    )
    parser.add_argument(
        "--dataset", default="ek100_cls", type=str, choices=["ek100_mir"]
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="number of crops in transforms for testing",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="dropout for the last linear layer",
    )
    parser.add_argument(
        "--num-classes",
        default=3806,
    ...
    parser.add_argument(
        "--optimizer", default="sgd", choices=["adamw", "sgd"], type=str
    )
    ...
    parser.add_argument(
        "--pickle-filename", default="", type=str, help="pickle filename to dump"
    )
    parser.add_argument(
        "--world-size",
        default=1,
...
def main(args):
    ...
    model = model_clip.VideoClassifier(
        model.visual, dropout=args.dropout_rate, num_classes=args.num_classes
    )
    model.cuda(args.gpu)
    ...
    if args.use_zero:
        ...
        optimizer = ZeroRedundancyOptimizer(
            optim_params, optimizer_class=opt_fn, **opt_kwargs
        )
    else:
    ...
    for s, all_preds in zip(["logits", " probs"], [all_logits, all_probs]):
        ...
        print(
            "[Average {s}] {dataset} * Acc@1 {top1:.3f} Acc@5 {top5:.3f}".format(
                s=s, dataset=dataset, top1=acc1, top5=acc5
            )
        )
