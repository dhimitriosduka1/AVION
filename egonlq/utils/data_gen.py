...
from base.transforms import init_video_transform_dict
import decord
import torch
...
class EpisodicNLQProcessor:
    def __init__(self, context_length):
        ...
        config = "/u/dduka/project/AVION/egonlq/configs/nlq.json"
                ...
                meta_dict.update(
                    {
                        anno_clip["clip_uid"]: {
                            ...
                            "video_start": clip_times[0],
                            "video_end": clip_times[1],
                        }
                    }
                )
    ...
    def process_data_tan(self, data, scope, args):
        ...
        for vid, data_item in tqdm(
            data.items(), total=len(data), desc=f"Processing episodic nlq {scope}"
        ):
    ...
    for i in range(max_num_clips):
        ...
        if s_idx < e_idx:
            new_visual_feature.append(
                np.mean(visual_feature[s_idx:e_idx].numpy(), axis=0)
            )
        else:
...
def _get_video_text_feats(meta_dict_scope, clip_uid, num_frames, transforms, data_dir):
    try:
        fps = 1.87
...
def dataset_gen(data, vfeat_lens, scope, args):
    dataset = []
    ...
    for indx, record in tqdm(enumerate(data), desc="dataset_gen_EgoVLP"):
        ...
        if "video_frame_path" in record["meta_dict_scope"][record["vid"]]:
            ...
            video = _get_video_text_feats(
                record["meta_dict_scope"],
                record["vid"],
                record["num_frame"],
                record["transforms"],
                record["data_dir"],
