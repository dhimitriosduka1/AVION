...
class NaturalLanguageQueries(TextVideoDataset):
    ...
    def _get_video_text_feats(self, item):
        ...
        imgs = self.visual_feature_sampling(imgs, max_num_clips=256)
        ...
        meta_arr = {
            "video_uid": sample[0],
            "clip_uid": sample[1],
            "video_start": sample[2],
            "video_end": sample[3],
            "data": video_fp,
            "dataset": self.dataset_name,
        }
        return {"video": imgs, "text": text, "meta": meta_arr}
...
if __name__ == "__main__":
    ...
    for i, _ in tqdm(enumerate(range(len(dataset)))):
        ...
        if item["meta"]["clip_uid"] in test_dict:
            if (
                test_dict[item["meta"]["clip_uid"]][0] != item["meta"]["video_start"]
                or test_dict[item["meta"]["clip_uid"]][1] != item["meta"]["video_end"]
            ):
                c += 1
        test_dict.update(
            {
                item["meta"]["clip_uid"]: (
                    item["meta"]["video_start"],
                    item["meta"]["video_end"],
                )
            }
        )
