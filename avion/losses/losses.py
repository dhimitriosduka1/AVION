...
def gather_features_and_scales(
    image_features,
    text_features,
    logit_scale,
    local_loss=False,
    ...
    def gather_tensor(tensor):
        if use_horovod:
            if gather_with_grad:
                ...
                with torch.no_grad():
                    all_tensors = hvd.allgather(tensor)
                if not local_loss:
                    chunks = list(all_tensors.chunk(world_size, dim=0))
                    chunks[rank] = tensor
                    all_tensors = torch.cat(chunks, dim=0)
                return all_tensors
        else:
            if gather_with_grad:
    ...
    all_image = gather_tensor(image_features)
    all_text = gather_tensor(text_features)
    all_scales = gather_tensor(logit_scale)

    return all_image, all_text, all_scales
...
class ClipLossWithModulatedTemperature(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale):
        ...
        if logit_scale.ndim == 1:
            logit_scale = logit_scale.unsqueeze(1)

        if self.world_size > 1:
            all_image_features, all_text_features, all_scales = gather_features(
                image_features,
                text_features,
                logit_scale,
                self.local_loss,
            ...
            if self.local_loss:
                ...
                logits_per_image = all_scales * (all_image_features @ all_text_features.T)
                logits_per_text = all_scales * (all_text_features @ all_image_features.T)
        else:
            logits_per_image = logit_scale * (image_features @ text_features.T)
            logits_per_text = logit_scale * (text_features @ image_features.T)
        ...
        return {"loss": total_loss, "clip_acc": acc, "logit_scale": logit_scale.mean().item(), "logit_scale_min": logit_scale.min().item(), "logit_scale_max": logit_scale.max().item()}
