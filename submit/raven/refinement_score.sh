...
#SBATCH -o /ptmp/dduka/work/logs/avion/segment_ranking_%j.out
#SBATCH -e /ptmp/dduka/work/logs/avion/segment_ranking_%j.err
#SBATCH --job-name=segment_ranking
#SBATCH --nodes=1
...
INPUT_MANIFEST="${INPUT_MANIFEST:?Set INPUT_MANIFEST to the Qwen-refined manifest path}"
LAVILA_CHECKPOINT="${LAVILA_CHECKPOINT:-}"
EGOVLP_CHECKPOINT="${EGOVLP_CHECKPOINT:-}"
...
OUTPUT_DIR="${OUTPUT_DIR:-/ptmp/dduka/work/segment_ranking/poc_test_csv_single_gpu}"
...
python -u tools/refinement/score_segments.py "${SCORE_ARGS[@]}" --log-every "$LOG_EVERY"
...
if (( RANK_SHARDS <= 1 )); then
    ...
    log "splitting scored candidates into $RANK_SHARDS ranking shards under $RANK_SHARD_DIR"
    ...
    log "created ${#SCORE_SHARDS[@]} score shards"
