#!/bin/bash -l
#SBATCH -o /dais/fs/scratch/dduka/logs/avion/iou_nav_%A_%a_%x_%j_%N.out
#SBATCH -e /dais/fs/scratch/dduka/logs/avion/iou_nav_%A_%a_%x_%j_%N.err
...
RUN_BASE=DUAL_ENCODER
RUN_NAME=${RUN_BASE}_IoU_${IoU}_NOUN_AND_VERBS_FROM_SCALED_2.1_DAIS
...
srun --cpu_bind=v --accel-bind=gn torchrun \
    ...
    --use-flash-attn \
    --train-metadata /dais/fs/scratch/dduka/databases/ego4d/iou_noun_and_verbs_from_scaled/ego4d_train_iou_${IoU}.pkl \
    --grad-checkpointing \
    --use-fast-conv1 \
    --batch-size 512 \
    --freeze-temperature \
